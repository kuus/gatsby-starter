...
import { LinkListItem, GroupListItem } from "./components";
...
export default class ListItem extends React.Component {
  ...
  render() {
    ...
    const color = "#4e5c6e";
    const arrowUp = this.state.hover ? (
      <AlignedSpan>
        <ArrowUp size="18" color={color} onClick={this.onClickUp} />
      </AlignedSpan>
    ) : (
      ""
    );
    const arrowDown = this.state.hover ? (
      <AlignedSpan>
        <ArrowDown size="18" color={color} onClick={this.onClickDown} />
      </AlignedSpan>
    ) : (
      ""
    );
    const deleteBtn = this.state.hover ? (
      <AlignedSpan>
        <Trash size="17" color={color} onClick={this.onClickDelete} />
      </AlignedSpan>
    ) : (
      ""
    );
